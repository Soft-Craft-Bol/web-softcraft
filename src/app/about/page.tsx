    ...
    <>
      ...
      <div className="mx-auto w-[min(100%-2rem,72rem)] py-14 sm:py-20">
        <div className="grid items-start gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-12">
          <blockquote className="rounded-2xl border border-magenta/40 bg-gradient-to-b from-panel2 to-plum p-8 shadow-[0_0_40px_rgba(255,46,136,0.15)] sm:p-10">
          ...
          <div>
            ...
            <ul className="mt-5 grid gap-4 sm:grid-cols-2">
              {CAPABILITIES.map((item) => (
                <li key={item.title} className="rounded-xl border border-white/12 bg-panel p-5 transition-colors hover:border-coral/60">
        ...
        <div className="mt-16">
          ...
          <ol className="mt-8 grid gap-5 md:grid-cols-3">
            {VALUES.map((value) => (
              <li key={value.index} className="relative rounded-2xl border border-white/12 bg-panel p-7 transition-colors hover:border-gold/50">
